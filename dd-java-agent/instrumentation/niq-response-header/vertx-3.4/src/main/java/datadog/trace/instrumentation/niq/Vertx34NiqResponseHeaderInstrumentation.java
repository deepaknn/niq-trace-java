...
public final class Vertx34NiqResponseHeaderInstrumentation extends InstrumenterModule.Tracing
    implements Instrumenter.ForTypeHierarchy, Instrumenter.HasMethodAdvice {

  private static final Logger log = LoggerFactory.getLogger(Vertx34NiqResponseHeaderInstrumentation.class);

  public Vertx34NiqResponseHeaderInstrumentation() {
    super("niq-response-header", "vertx", "vertx-3.4");
  }
  ...
  public void methodAdvice(MethodTransformer transformer) {
    transformer.applyAdvice(
        ...
        Vertx34NiqResponseHeaderInstrumentation.class.getName() + "$ResponseAdvice");
  }
  ...
  public static class ResponseAdvice {
    ...
    public static void injectHeader(@Advice.This final Object response) {
      try {
        if (INJECTED.get()) {
          log.trace("[NIQ-VERTX34] Header already injected");
          return;
        }
        ...
        if (headersEnded) {
          log.trace("[NIQ-VERTX34] Headers already sent");
          return;
        }

        AgentSpan span = AgentTracer.activeSpan();
        if (span == null) {
          log.trace("[NIQ-VERTX34] No active span found");
          return;
        }
        ...
        if (spanId == 0 || traceId == null || traceId == DDTraceId.ZERO) {
          log.trace("[NIQ-VERTX34] Invalid span or trace ID");
          return;
        }
        ...
        // Call putHeader method: response.putHeader("current-span-id", traceparent)
        ...
        log.debug("[NIQ-VERTX34] Successfully injected current-span-id header: {}", traceparent);
      } catch (Exception e) {
        log.warn("[NIQ-VERTX34] Error injecting header", e);
      }
    }
