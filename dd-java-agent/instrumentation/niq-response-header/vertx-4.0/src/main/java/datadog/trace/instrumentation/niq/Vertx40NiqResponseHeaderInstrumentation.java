...
public final class Vertx40NiqResponseHeaderInstrumentation extends InstrumenterModule.Tracing
    implements Instrumenter.ForTypeHierarchy, Instrumenter.HasMethodAdvice {

  private static final Logger log = LoggerFactory.getLogger(Vertx40NiqResponseHeaderInstrumentation.class);

  public Vertx40NiqResponseHeaderInstrumentation() {
    super("niq-response-header", "vertx", "vertx-4.0");
  }
  ...
  public void methodAdvice(MethodTransformer transformer) {
    transformer.applyAdvice(
        ...
        Vertx40NiqResponseHeaderInstrumentation.class.getName() + "$ResponseAdvice");
  }
  ...
  public static class ResponseAdvice {
    ...
    public static void injectHeader(@Advice.This final Object response) {
      try {
        if (INJECTED.get()) {
          log.trace("[NIQ-VERTX40] Header already injected");
          return;
        }
        ...
        if (headersEnded) {
          log.trace("[NIQ-VERTX40] Headers already sent");
          return;
        }

        AgentSpan span = AgentTracer.activeSpan();
        if (span == null) {
          log.trace("[NIQ-VERTX40] No active span found");
          return;
        }
        ...
        if (spanId == 0 || traceId == null || traceId == DDTraceId.ZERO) {
          log.trace("[NIQ-VERTX40] Invalid span or trace ID");
          return;
        }
        ...
        log.debug("[NIQ-VERTX40] Successfully injected current-span-id header: {}", traceparent);
      } catch (Exception e) {
        log.warn("[NIQ-VERTX40] Error injecting header", e);
      }
    }
